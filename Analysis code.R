...
setwd(dir = "D:/0. Previous/2018-20 in Roh lab/Data/Microbiome_LSG/2019/190814 Figure_1526 samples/")
library(ggplot2); library(reshape2); library(MASS); library(ggpubr)
library(vegan); library(ggforce); library(ggsignif); library(ggthemes)
library("extrafont") ; library(Hmisc) ;library(gtable); library(grid) 
library(caret) ; library(ROCR) ; library(pROC) ;library(plotROC) 
library(RJSONIO) ; library(reshape) ;library(NbClust) ;library("RColorBrewer")
...
#### 2. Input ####
# Input
bugs <- read.csv("input/bugs_1526.csv", row.names = 1) # 533 microbes x 1526 samples
  NOD <- rowSums(bugs>0) # Number of detection for every microbes across whole samples
meta <- read.csv("input/meta_1526.csv", row.names = 1) # Metadata for 1526 samples
...
ph <- t(read.csv("input/phylum.csv", row.names = 1)) ; ph <- as.data.frame(ph[order(rownames(ph)),]) # Phylum level composition
...
re <- read.csv("input/replicated.csv", row.names = 1) # replicated sample
...
# Melting
all_melt <- reshape::melt(bugs2, id.vars = "microbe") 
...
div.melt <- reshape::melt(data = meta, id.vars = c("Shannon", "Evenness", "Richness", "Human.Read.Fraction"), 
...
# Getting F.nucleatum abundance data
Fn_meta <- as.data.frame(t(bugs["Fusobacterium_nucleatum",Fn_pos]))
Fn_meta$External.ID <- rownames(Fn_meta)
Fn_meta <- cbind(Fn_meta, 
                 subset(meta.order[meta.order$Fn.proximity=="Detected",], 
                        select = c("Condition", "Type", "Activity", 
                                   "Sex", "consent_age", "Site", "Participant.ID")))

# Sorting F. nucleatum-expereinced subject 
Fn_subj <- Fn_meta[!duplicated(Fn_meta$Participant.ID),] # 20 subjects
meta_subj <- meta[!duplicated(meta$Participant.ID),]
...
# Profiling sample information by F. nucleatum observation
    ...
    # 2-2) Subjects
    table(Fn_subj$Type) # nonIBD 4 : UC 5 : CD 11
    table(meta_subj$Type) # nonIBD 26 : UC 30 : CD 50
    
    # 3) Site
    # 3-1) Samples
    table(Fn_meta$Site) 
      # Cedar 12 : Cincinnati 13 : Emory 1 : MGH 13 : MGH Pediatrics 2
    table(meta$Site) 
      # Cedar 390 : Cincinnati 448 : Emory 49 : MGH 426 : MGH Pediatrics 213
    
    # 3-2) Subjects
    table(Fn_subj$Site) 
      # Cedar 3 : Cincinnati 9 ; Emory 1 ; MGH 4 ; MGH Pediatrics 2
    table(meta_subj$Site) 
      # Cedar 27 : Cincinnati 31 ; Emory 4 ; MGH 28 ; MGH Pediatrics 16
    ...
    # 4-2) Subjects
    table(Fn_subj$Condition) # nonIBD 4 ; IBD 15
...
### Pie ###
# # Proportion of IBD vs. non-IBD in whole samples    
# tiff("figures/pie_condition.tiff", 
#      res = 400, width = 600, height = 600); ggplot(meta, aes(x="1", y = Condition, fill=Condition)) + 
#   geom_bar(stat = "identity", alpha=0.6) + xlab("") + ylab("") +
#   scale_fill_manual(values = col2) + 
#   coord_polar(theta="y") + 
#   theme_void() + theme(legend.position = "none") ; dev.off()
# 
# # Proportion of IBD vs. non-IBD in whole Participants
# tiff("figures/pie_condition_subjects.tiff", 
#      res = 400, width = 600, height = 600); ggplot(meta_subj, aes(x="1", y = Condition, fill=Condition)) + 
#   geom_bar(stat = "identity", alpha=0.6) + xlab("") + ylab("") +
#   scale_fill_manual(values = col2) + 
#   coord_polar(theta="y") + 
#   theme_void() + theme(legend.position = "none") ; dev.off()
# 
# # Proportion of IBD vs. non-IBD in whole samples
# tiff("figures/pie_condition_upon_detection.tiff", 
#      res = 400, width = 600, height = 600) ; ggplot(Fn_meta, aes(x="1", y = Condition, fill=Condition)) + 
#   geom_bar(stat = "identity", alpha=0.9) + xlab("") + ylab("") +
#   scale_fill_manual(values = col2) + coord_polar(theta="y") + theme_void() + theme(legend.position = "none") ; dev.off()
# 
# # Proportion of IBD vs. non-IBD in whole samples
# tiff("figures/pie_condition_upon_detection_subject.tiff", 
#      res = 400, width = 600, height = 600) ; ggplot(Fn_subj, aes(x="1", y = Condition, fill=Condition)) + 
#   geom_bar(stat = "identity", alpha=0.9) + xlab("") + ylab("") +
#   scale_fill_manual(values = col2) + coord_polar(theta="y") + theme_void() + theme(legend.position = "none") ; dev.off()
...
# Make contingency table   
    # 1) IBD vs. F. nucleatum (Subject)
cont.subj <- data.frame(all=as.vector(table(meta_subj$Condition)),
                        F.n=as.vector(table(Fn_subj$Condition))) ; rownames(cont.subj) <- c("nonIBD", "IBD")

cont.subj$Non <- cont.subj$all - cont.subj$F.n
cont.subj$all <- cont.subj$all/106
cont.subj$F.n <- cont.subj$F.n/19
cont.subj$Non <- cont.subj$Non/87
cont.subj$Type <- rownames(cont.subj)
cont.subj$all <- NULL

  # 2) IBD vs. F. nucleatum (Samples)
cont.sample <- data.frame(all=as.vector(table(meta$Condition)),
                        F.n=as.vector(table(Fn_meta$Condition))) ; rownames(cont.sample) <- c("nonIBD", "IBD")

cont.sample$Non <- cont.sample$all - cont.sample$F.n
cont.sample$all <- cont.sample$all/1526 
cont.sample$F.n <- cont.sample$F.n/41
cont.sample$Non <- cont.sample$Non/1485
cont.sample$Type <- rownames(cont.sample)
cont.sample$all <- NULL


cont.subj$Type <- factor(cont.subj$Type, levels=c("nonIBD", "IBD"))
cont.sample$Type <- factor(cont.sample$Type, levels=c("nonIBD", "IBD"))

# Melting to draw bar graph
m.subj <- reshape::melt(cont.subj, id.vars="Type", measure.vars=c("Non", "F.n"))
m.sample <- reshape::melt(cont.sample, id.vars="Type", measure.vars=c("Non", "F.n"))


# tiff("figures/bar_condition_subject_Fn.tiff",
#      res = 200, width = 350, height = 600, units = "px");ggplot(m.subj, aes(variable, value, fill=Type)) +
#   xlab("") + ylab("")+
...
#         axis.ticks.x = element_blank(),
#         axis.text.y = element_text(color="black", size=rel(0.8)));dev.off()
# 
# tiff("figures/bar_condition_sample_Fn.tiff",
#      res = 200, width = 350, height = 600, units = "px");ggplot(m.sample, aes(variable, value, fill=Type)) +
#   xlab("") + ylab("")+
#   geom_bar(stat = "identity", alpha=0.9) +
#   theme_void() +
#   scale_fill_manual(values = col2) +
...
Fn_meta$Fusobacterium_nucleatum
Fn_meta.order <- unique(Fn_meta$Participant.ID)
m.Fn_meta <- reshape::melt(Fn_meta, id.vars=c("Participant.ID", "External.ID", "Condition"),
              measure.vars="Fusobacterium_nucleatum")

# 1) By subject
m.Fn_meta$Participant.ID <- factor(m.Fn_meta$Participant.ID, levels=Fn_meta.order)
# tiff(file = "figures/Fn_upon_detection_bySuject.tiff", width = 800, height = 600, units = "px", res = 200) ;ggplot(m.Fn_meta, aes(Participant.ID, log10(value), 
#                       color=Condition))+
#   xlab("")+ylab("")+
#   geom_point(alpha=0.7) + 
#   scale_y_continuous(limits = c(-4, 2)) + scale_color_manual(values = col2) + 
#   theme_classic2()+
#   theme(panel.background = element_rect(color="black", linetype = "solid", size = 1),
#         panel.grid = element_blank(),
#         panel.grid.major.y = element_line(color = "gray95"),
#         axis.text.x = element_text(color="black",angle=90, vjust=0.3, hjust=1),
#         axis.ticks.x = element_line(), #blank(),
#         legend.position = c(0.82, 0.82),
#         legend.title = element_blank(),
#         legend.direction = "vertical",
#         legend.box.background = element_rect(color="transparent", fill = "transparent"),
#         legend.key.height = unit(1,"line"),
#         legend.text = element_text(size=rel(1.2))); dev.off()
...
# tiff(file = "figures/Fn_upon_detection.tiff", width = 800, height = 600, units = "px", res = 200) ; ggplot(Fn_meta, aes(External.ID, log10(Fusobacterium_nucleatum))) + 
#   xlab("") + ylab("") + 
...
# Contingency table: F. nucleatum observation by IBD (Subjects)
Fn.2x2_IBD <- data.frame(F.n=c("Observed", "None"), nonIBD=c(4, 26), IBD=c(15, 80)) ; Fn.2x2mat <- as.matrix(Fn.2x2_IBD[,2:3])
fisher.test(x = Fn.2x2mat, conf.int = 0.95, alternative = "less") # Odd ratio=0.8217814 ; p-value = 0.5006

# Contingency table: F. nucleatum observation by IBD (Samples)
...
mark.melt <- reshape::melt(mark.tmp, id.vars = c("External.ID","Condition", "Week", "Fn.directionality", "Fn.directionality.strict"), measure.vars = mark.list)
...
# Melting data
melt_sum <- reshape::melt(mark.meta, id.vars = c("External.ID", "Fn.directionality", "Fn.directionality.strict", "Condition"), measure.vars = c("sumIBD", "sumNON"))
melt_spec <- reshape::melt(mark.meta, id.vars = c("External.ID", "Fn.directionality", "Fn.directionality.strict", "Condition"), measure.vars = c("specIBD", "specNON"))
...
#  Distribution of IBD/non-IBD marker species over time
  ...
  # 3) IBD marker 2 = IBD markers, non-positively correlated with F. nucleatum
    ...
    # Melting
    mark.melt <- reshape::melt(mark.meta, id.vars = c("Condition", "Fn.directionality.strict"), measure.vars = c("sumNON", "nonPos_IBD.sum", "Pos_IBD.sum")) ; mark.melt <- mark.melt[is.na(mark.melt$Fn.directionality.strict)==F,]
...
all_markers <- c(ibd.mark, non.mark) ; all_markers <- data.frame(microbe=all_markers) ; all_markers$Condition <- ifelse(all_markers$microbe %in% non.mark, yes = "nonIBD", no = "IBD")
  melt_marker <- reshape::melt(mark.meta, id.vars = c("Participant.ID", "External.ID","Week", "Condition", "Fn.directionality.strict"), measure.vars = c(ibd.mark, non.mark))
...
# Pairwise conversion by reshape::melt()
# remove value=0
melt.IBD <- subset(reshape::melt(IBDm), value!=0) ; melt.non <- subset(reshape::melt(nonIBDm), value!=0) ;rm(melt.IBD, melt.non, IBDm, nonIBDm)
...
int.con.melt <- reshape::melt(Inter.condition) ; colnames(int.con.melt) <- c("Condition", "Distance")
...
int.exp.melt <- reshape::melt(Inter.exp) ; colnames(int.exp.melt) <- c("Condition", "Distance")
...
int.pp.melt <- reshape::melt(inter.prepost) ; colnames(int.pp.melt) <- c("Condition", "Distance")
...
melt_con_spec <- reshape::melt(tb_con, id.vars = "Condition", measure.vars = c("post_spec", "pre_spec"))
melt_con_sum <- reshape::melt(tb_con, id.vars = "Condition", measure.vars = c("post_sum", "pre_sum"))
...
melt_exp <- reshape::melt(Prob_Exp, id.vars = c("Predicted", "Condition"), measure.vars = c("Shannon", "Evenness", "Richness", "IBD", "nonIBD", "CRC", "IBD/nonIBD"))
...
melt_inno <- reshape::melt(Prob_Inno, id.vars = c("Predicted", "Condition"), measure.vars = c("Shannon", "Evenness", "Richness", "IBD", "nonIBD", "CRC", "IBD/nonIBD"))
head(Prob_Inno)

  # Contingency table for F. nucleatum-innocent samples 
    # Posterior probability-based putative posterior/prior group
    # Condition
  
  with(Prob_Inno, table(Condition, Predicted))
  # Prior 378 vs. Posterior 831
  # non-IBD 337 vs. IBD 872
  
  melt_inno_post_prob <- reshape::melt(Prob_Inno, 
                                       id = c("Participant.ID", "Predicted", "Condition"), 
                                       measure.vars = "Post_prob")
  table(melt_inno_post_prob$Predicted) # Prior 378, Posterior 831
  
  # Casting to calculate median probability per individual
  cast_inno <- cast(melt_inno_post_prob, formula = Participant.ID + Condition ~variable, median)
  cast_inno_order <- cast_inno$Participant.ID[order(cast_inno$Post_prob, decreasing = T)]
  cast_inno$Predicted <- ifelse(cast_inno$Post_prob>0.5, "Posterior", "Prior")
    table(cast_inno$Predicted, cast_inno$Condition) # Pre/Post vs. nonIBD/IBD
  
  m_inno_prob_50 <- subset(melt_inno_post_prob, value > 0.5)
  unique(m_inno_prob_50$Participant.ID)
    
# tiff("figures/indiviausl_posterior_probability.tiff",
#    res = 180, height = 800, width = 1200, units = "px"); ggplot(Prob_Inno, aes(Participant.ID, Post_prob))+
#     xlab("Participant ID") + ylab("Posterior probability")+
#     geom_hline(yintercept = 0.5, alpha=0.5, linetype="dotted", size=1)+
#     geom_boxplot(alpha=0.2, outlier.alpha = 0)+
#     geom_point(alpha=0.6, aes(color=Condition), size=rel(0.9)) +
#     theme_classic()+
#     scale_color_manual(values = col2)+
#     scale_x_discrete(limits=cast_inno_order)+
#     theme(legend.position = "bottom",
#           legend.text = element_text(size=rel(0.9)),
#           axis.title = element_text(size=rel(1.1), vjust=1),
#           axis.text.x = element_text(angle=90, size = rel(0.7), vjust=0.3, hjust=1, color="black"),
...
Prob.melt <- reshape::melt(Prob_all, id.vars = c("Participant.ID", "Condition", "Week"), measure.vars = c("Post_prob", "Shannon2"))
...
#### 11-1) Determining number of clusters (k-means) ####
set.seed(12345)
...
tmp_melt <- reshape::melt(tmp.df, id.vars = "kmean",  measure.vars = c("Max.withinss", "Min.withinss", "Avg.withinss", "tot.withinss", "betweenss"))
...
str(pcoa.dynamic.df)
dat1 <- reshape::melt(pcoa.dynamic.df, id.vars = "Cluster", measure.vars = c("Condition", "Core", "Classifier", "pre.post", "Pathogenic"))
...
# Sum of abundance
cl_sum <- reshape::melt(cl_df, id.vars = c("External.ID", "Condition", "Fn.directionality.strict", "Fn.pre.post.simple", "Fn.experience", "Post_prob"), measure.vars = paste0("V", 1:km.size)) 
cl_sum$variable <- factor(cl_sum$variable, labels = c(1:km.size)) ; cl_sum$variable

# Number of detected cluster components
cl_spec <- reshape::melt(cl_df, id.vars = c("External.ID", "Condition", "Fn.directionality.strict", "Fn.pre.post.simple", "Fn.experience", "Post_prob"), measure.vars = paste0("V", (km.size+1):(km.size*2)))
cl_spec$variable <- factor(cl_spec$variable, labels = c(1:km.size)) ; cl_spec$variable

# Percentage for detected cluster components
cl_spec_perc <- reshape::melt(cl_df, id.vars = c("External.ID", "Condition", "Fn.directionality.strict","Fn.pre.post.simple", "Fn.experience", "Post_prob"), measure.vars = paste0("V", (km.size*2+1):(km.size*3)))
...
colnames(re)[seq(2, 44, 2)]

meta[meta$External.ID %in% colnames(re)[seq(2, 44, 2)],]
...
# Melting 
melt1 <- reshape::melt(df1, id.vars = "microbes") ; melt1$Matched <- reshape::melt(df2, id.vars = "microbes", measure.vars = colnames(df2)[1:22])$value ;
...
# Melting
melt_1_number <- reshape::melt(pair.df, id.vars = "Pair.ID", measure.vars = c("Full_species", "Half_species"))
...
melt_2_mean <- reshape::melt(pair.df, id.vars = "Pair.ID", measure.vars = c("Mean_twice", "Mean_once"))
...
melt_3_median <- reshape::melt(pair.df, id.vars = "Pair.ID", measure.vars = c("Med_twice", "Med_once"))
